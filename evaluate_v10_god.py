...
from expectimax_v10_god import get_v10_god_move, clear_transposition_table

def run_v10_tournament(n_games=5):
    print(f"\n👑 ROZPOZCZYNAM TURNIEJ MISTRZOWSKI V10 (8M CACHE, PRISTINE SINGLE THREAD, DEPTH 6)...\n")

    # Warmup Numba C++
    dummy_b = np.array([[2, 4, 8, 16], [32, 64, 128, 256], [512, 1024, 2048, 4096], [0, 0, 2, 4]], dtype=np.int32)
    get_v10_god_move(dummy_b)
    print("✅ Kompilacja V10 C++ z 8-Milionowym Cachem zakończona sukcesem!\n")
    ...
    for g in range(n_games):
        ...
        while not done:
            ...
            action, depth_used = get_v10_god_move(env.board, valid_mask=masks)
    ...
    print(f"\n{'='*65}")
    print(f"  👑 OSTATECZNE WYNIKI MISTRZA V10 GOD ENGINE")
    print(f"{'='*65}")
