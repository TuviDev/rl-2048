...
import numpy as np
import random
from numba import njit, uint64, uint32, int64, boolean, types
...
class Game2048BitboardEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self._update_board_np()
        current_max = int(self.board_np.max()) if self.board_np.max() > 0 else 0
        ...
        empty_count = np.sum(self.board_np == 0)
        reward += empty_count * 0.4
