...
from expectimax_v8_god10 import get_v8_god_move, clear_transposition_table

def run_v8_tournament(n_games=3):
    print(f"\n👑 ROZPOZCZYNAM TURNIEJ GOD MODE V8 (DEPTH 8-10, PROBABILITY PRUNING)...\n")

    # Warmup
    dummy_b = np.array([[2, 4, 8, 16], [32, 64, 128, 256], [512, 1024, 2048, 4096], [0, 0, 2, 4]], dtype=np.int32)
    get_v8_god_move(dummy_b)
    print("✅ Kompilacja V8 z Głębokością 10 C++ zakończona sukcesem!\n")
    ...
    for g in range(n_games):
        ...
        while not done:
            ...
            action, depth_used = get_v8_god_move(env.board, valid_mask=masks)
    ...
    print(f"\n{'='*65}")
    print(f"  👑 OSTATECZNE WYNIKI MISTRZA V8 DEPTH 8-10")
    print(f"{'='*65}")
