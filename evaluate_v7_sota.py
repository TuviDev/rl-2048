...
from expectimax_v7_sota import get_v7_god_move, clear_transposition_table

def run_v7_tournament(n_games=3):
    print(f"\n👑 ROZPOZCZYNAM TURNIEJ WORLD RECORD V7 (Smoothness + Monotonicity + Depth 7)...\n")

    # Warmup
    dummy_b = np.array([[2, 4, 8, 16], [32, 64, 128, 256], [512, 1024, 2048, 4096], [0, 0, 2, 4]], dtype=np.int32)
    get_v7_god_move(dummy_b)
    print("✅ Kompilacja V7 SOTA Engine zakończona sukcesem!\n")
    ...
    for g in range(n_games):
        ...
        while not done:
            ...
            action, depth_used = get_v7_god_move(env.board, valid_mask=masks)
    ...
    print(f"\n{'='*65}")
    print(f"  👑 OSTATECZNE WYNIKI BOKSERA MISTRZOWSKIEGO V7 SOTA")
    print(f"{'='*65}")
